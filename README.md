...
Fetch Data: GET http://127.0.0.1:8000/api/data


Option B: Running via CLI (Standalone)
If you just want to run a single physical simulation pass without the web server:
...
rf_propagation.py
RF physics layer (Path loss, atmosphere, antenna)
study_area.py
Geographic framework and grid generation
link_budget.py
3GPP link budget calculations
generate_walker.py
Constellation payload generation

Note: For deeper technical explanations of the physical models and formulas used, refer to TECHNICAL_README.md.
Configuration
To customize the physical simulation parameters, edit the configuration block inside main_simulator.py:
...
Running Tests
The project includes a comprehensive test suite covering the core physical algorithms.
...
(Tests cover RF propagation, path loss, atmospheric modeling, grid generation, distances, and angles)[cite: 1].
Troubleshooting
Windows Users: UnicodeEncodeError or UnicodeDecodeError
If your simulation crashes with a codec error while attempting to process or print the ASCII banner, it is due to the Windows terminal default cp1252 encoding[cite: 1].
Fix: This has been patched by forcing utf-8 on process streams[cite: 1]. If you still experience issues, force the environment encoding in your terminal before running the server[cite: 1]:
...
If the backend background engine fails during the final phase of the simulation, check your local drive storage space. The simulation data structures export large datasets; cleaning up previous ntn_coverage_analysis.csv files or clearing the system Recycle Bin will resolve the write restriction.
References
3GPP TS 38.811 / TR 38.811: NR and NG-RAN Overall Description (NTN) & Use Cases[cite: 1]
ITU-R P.618 / P.676: Propagation in line-of-sight systems & Atmospheric attenuation[cite: 1]
Core Libraries: Skyfield (Orbital mechanics), FastAPI (REST API), NumPy, Pandas, Matplotlib[cite: 1].
Author: Damian Stochla[cite: 1]
Version: 1.1.0[cite: 1]
License: Academic use - please cite this project if used in research or thesis work[cite: 1].
```
